...
void __no_inline_not_in_flash_func(flash_range_erase)(uint32_t flash_offs, size_t count, bool enable_xip) {
...
void __no_inline_not_in_flash_func(flash_range_program)(uint32_t flash_offs, const uint8_t *data, size_t count, bool enable_xip) {
#ifdef PICO_FLASH_SIZE_BYTES
    //hard_assert(flash_offs + count <= PICO_FLASH_SIZE_BYTES);
#endif
